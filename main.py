...
v28 Özellikleri:
  ...
  - Playwright: --no-sandbox (GitHub Actions uyumlu)
  - networkidle wait
...
RAKIP_FIRMALAR = [
    ...
    # v29'da eklenen firmalar (29 Nisan 2026)
...
def linkten_url_bul(html_govde):
    """
    edoksis linkini bulur
    v29: Debug eklendi
    """
    if not html_govde:
        log.warning("❌ Mail içeriği boş")
        return None
    
    # Debug: İçerik var mı?
    log.info(f"📄 Mail içeriği uzunluğu: {len(html_govde)} karakter")
    
    # Debug: edoksis geçiyor mu?
    if "edoksis" in html_govde.lower():
        log.info("✅ 'edoksis' kelimesi bulundu")
    else:
        log.warning("⚠️ 'edoksis' kelimesi bulunamadı!")
    ...
    log.warning("❌ Regex ile link bulunamadı")
    return None
...
def bize_ait_mi_kontrol(icerik, html_govde):
    ...
    # Liman kontrolü (v29: SAN PEDRO, ABIDJAN)
    for liman in ELEME_LIMANLARI:
        liman_norm = turkce_normalize(liman)
        # POL yanında veya "Yükleme Limanı" yanında geçiyor mu?
        if re.search(rf"(?:POL|YUKLEME\s*LIMANI)[:\s-]*{re.escape(liman_norm)}", metin):
            return False, f"Eleme limanı: {liman}"
    ...
    # Yükleyici/Gönderici kontrolü (v29: DÜZELTME - Brisa ise bize ait değil)
    # SHIPPER = Brisa → İhracat faturası → Bize ait değil
...
def main():
    log.info("═══ Sistem başladı (IMAP + Service Account) v29 ═══")
